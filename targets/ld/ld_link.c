...
static void merge_label_into(struct link_incr_ctx* ctx, const char* name,
            struct bin_label* target, struct bin_label* from) {
    if(from->flags & BL_SECTION) {
        ...
        remap->section = name;
        remap->offset = (size_t) sm_get(&ctx->section_base_offsets, from->section);
        sm_put(&ctx->source_remaps, name, remap, true);
    } else if(from->flags & BL_UNDEF) {
        // Even if the new is undefined, our life is not necesarily simple
        if(from->flags & BL_EXPORTED) {
            if(target->flags & BL_EXPORTED) {
                ...
                target->section = strcpy_dup(from->section);
                target->offset = from->offset + (size_t) sm_get(&ctx->section_base_offsets, from->section);
